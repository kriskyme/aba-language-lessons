...
Next step for this Band: run `Generate_Lesson_Prompt_v2.5.md` for Lesson 2 against the approved plan, one lesson
...
**Step 2 - Generate lessons.** Run `Generate_Lesson_Prompt_v2.5.md` (pasted with
...
- **Room-neutral backlog (Quality Standards §D11, 2026-09-09):** these already-generated lessons still run a
  protocol or hook that rearranges the room or posts something on the walls. Rewrite each to its §D11
  equivalent (mapping table in `Rotation_Log.md`) when the lesson is next touched, updating its Band's
  rotation-log row in the same pass: Beginner `Module_1/Set_1/Lesson_1_WhatIsIt/` (Fishbowl inner circle);
  Intermediate `Module_1/Set_1/Lesson_1_NewBakery/` (Fishbowl inner circle),
  `Module_1/Set_1/Lesson_2_PortoFoodTour/` (Four-Corner Debate posting a statement in each corner),
  `Module_1/Set_1/Lesson_4_GreatGrandmother/` (Day 2's two facing circles - its packet already says "get into
  a small group," so the `.md` and packet currently disagree, and Rotating Partners resolves both); Advanced
  `Module_1/Set_1/Lesson_2_LivingTextbooks/` (Fishbowl) and `Lesson_3_KeyDeer/` (Four-Corner Debate posting
  four signs around the room); Proficient `Module_1/Set_1/Lesson_1_LowerNinthWard/` (the same four signs).
  The sign- and sheet-posting ones are §D8 violations too.
  Advanced `Lesson_4_GlassBender/` was rewritten 2026-09-09 as the worked example. Do not generate new
  content with the old pattern.
...
- **Generate Proficient Module 1 Set 1's remaining lessons (2-4)** - Lesson 1 is generated; Lessons 2-4 are still
  planned only. Run `Generate_Lesson_Prompt_v2.5.md` one lesson at a time, per the Generation workflow below,
