...
| `Generate_Student_Packet_Prompt_v1.1.md`           | Takes one completed lesson and produces a single, print-ready, black-and-white student handout (self-contained HTML), mirroring Passage Reading's print prompt (same shared base stylesheet, in `shared/Student_Packet_Style_Guide.md`, same star-rating system), adapted for a real-source lesson: a plain citation box instead of a printed passage, a fillable Listening Notes organizer instead of an annotation key, two parts (Unit \_A Listening / Unit \_B Speaking) instead of Unit \_A/\_B by calendar day. As of v1.1, the opening masthead carries a top-right `.masthead-tag` reading "Listening & Speaking."                                                                                                                                                                                                       | Current (v1.1); six packets generated (Intermediate 1-4, Advanced 1-2). The two Set 1 Lesson 1 packets (NewBakery, LostKitchen) were hand-corrected 2026-09-06 to add the `.masthead-tag` and to match the shared base stylesheet exactly (idiom styling, `.task-block .instr-line` spacing, vocab-list column width, one-line `font-family` formatting) - see `shared/Changelog.md`. The remaining four (PortoFoodTour, Backpack, GreatGrandmother, LivingTextbooks) still predate the `.masthead-tag` and likely carry the same base-stylesheet drift; not yet swept |
...
| `Generate_Assessment_Student_Packet_Prompt_v1.md` | Takes a completed Assessment (Part A + Part B) and produces a single, print-ready student handout: a Listening Test section (one page per task Level, citation box, fillable notes organizer, that Level's items only) and a Speaking Task section (a plain instruction card per Level - topic, target length, submission info). Strips every answer key, point value, holistic pass note, and rubric - none of that is student-facing. Mirrors `Generate_Student_Packet_Prompt_v1.1.md`'s translation approach and reuses the shared base stylesheet/classes. | Current (v1); one packet generated (Intermediate Set 1) |
...
**Step 2 - Generate lessons.** Run `Generate_Lesson_Prompt_v1.md` against the approved
