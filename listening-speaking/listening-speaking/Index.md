...
- **Two-source compare backlog (Quality Standards §C9 enough-to-compare floor, 2026-09-09):** Advanced Set 1's
  `Module_1/Set_1/Lesson_2_LivingTextbooks/`, `Lesson_3_KeyDeer/`, and `Lesson_4_GlassBender/` packets (and their
  `.md` Level 7 tasks) still print Task D's two sources as a one-sentence summary each, the shape the stem sweep
  left. Rebuild each to at least three verbatim excerpts per source on shared subjects, in whichever of the two
  §C layouts fits that lesson's sources, re-fetching both sources for exact wording, when the lesson is next
  touched. Lesson 1 (LostKitchen, base and `Set_1_T`) was rebuilt 2026-09-09 as the worked example.
