# Listening/Speaking Assessment Student Print Formatting Prompt (v2.2)
...
the base `.match-list`/`.match-row` classes (§B). A predict-then-confirm item stays one numbered item with both circles in one
...
Style Guide §A-§D, this modality's §H.1 classes (`.citebox`, `.task-instr`, `.qitem`), the base
`.match-list`/`.match-row` compare classes and `.pic-options`/`.pic-option` picture-choice grid (§B), and the shared `.checklist` class (§H.4). Do not include `.notes-table` or
