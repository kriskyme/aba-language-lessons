...
- **Multi-source tasks (Level 7, when the top task compares two real sources):** present both sources' material
  ...
  fair-use ceiling, on the same two or more subjects in both sources (Quality Standards §C9); a one-line summary
  of what a source covers is not enough, since the student cannot rewind the clip or open the second source
  mid-task. Choose the layout per lesson (Style Guide §F, on the base `.match-row` classes in §B): one `.match-row` per source with its excerpts
  under the source label, or, when both sources speak to the same subjects, one `.match-row` per subject with
  each source's excerpt on its own `.match-src` line. Each excerpt is that source's own words or a neutral
  paraphrase of what it says, never a description of its tone or framing and never a statement of the contrast
...
`.stop-flag`/`.stop-badge`, `.task-instr`, `.qitem`, `.mc-list`/`.mc-letter`,
`.time-list`, `.reader-copy`/`.reader-warn`), plus the base `.match-list`/`.match-row` compare classes and the
base `.pic-options`/`.pic-option` picture-choice grid from §B. Add `.refresher` and `.notes-table` to the packet's `@media print`
