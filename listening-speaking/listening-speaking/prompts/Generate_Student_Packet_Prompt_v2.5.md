# Listening/Speaking Student Print Formatting Prompt (v2.5)
...
- **Multi-source tasks (Level 7, when the top task compares two real sources):** present both sources' material
  ...
  mid-task. Choose the layout per lesson (Style Guide §C, §H.1): one `.match-row` per source with its excerpts
