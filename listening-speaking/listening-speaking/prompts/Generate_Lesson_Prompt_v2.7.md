...
- **Phase 1 hooks:** Visual Inquiry, Take a Side, Mystery Quote, K-W-L Chart. Every one of them orients before
  it asks (Quality Standards §D13): the hook names the topic, the situation, and what kind of clip is coming
  before its first question, and no question's answer lies only in the clip students have not heard yet. A
  Mystery Quote prints one line from the clip **after** naming the topic, and asks what that line makes students
  expect the clip to cover, never who is speaking or what has just happened. A Visual Inquiry captions its
  photograph; an uncaptioned one is allowed only where every question asked of it is answerable by looking.
...
1. **Phase 1 hook (Quality Standards §D13):** does it name the topic, the situation, and the kind of clip
   before its first question, with no question answerable only from the unheard clip (who is speaking, what has
   just happened, what the clip is about)?
2. **Source real and verifiable** (never fabricated), with a genuine transcript or captions confirmed before any
...
5. **Both halves of each Level's CSV row used:** the Listening half for Day 1 tasks, the Speaking half for Day 2
   tasks (0.1)?
6. **Target words and phrases (4-6) present in the real transcript**, correctly glossed, with any
   naturally-occurring opaque idiom glossed at every band?
7. **Direct quotation inside the `.md` within the fair-use ceiling** (one to two sentences at a time), the full
   transcript never reproduced there (0.3 item 4)?
8. **Every listen-for or watch-for task tagged with a consistent timestamp range** or, where unverifiable, the
   source's own segment structure (0.3 item 5)?
9. **Background Notes** occasional (1-3), factual, sourced, informational only, never tested (0.3 item 6)?
10. **Transcript file** present in the lesson folder with the same citation block, honest about any gap, never
   printed whole, and reaching the student packet only as Phase 5's bounded verification windows (0.3 items 4,
   8)?
11. **Day 1 Phase 6's Closing Transfer Check script** is a short invented paragraph written into the lesson (60-90
...
12. **Day 2 Phase 1's speaking skill** drawn from the real speakers' own language at a cited timestamp or, where
...
13. **Pronunciation feature** genuinely audible in the real source and modeled from its audio, not an invented
    example?
14. **Lowest task Level's fixed-frame items** carry a complete inline instruction (what to listen for, what fills
    the blanks), with a separate frame box only where a speak-it-aloud step anchors it?
15. **Fixed-output Levels (0.6; Quality Standards §D10):** where Level 1 or 2 is a task Level, its form (point,
    ...
    once per day?
16. **Answer notes:** every exemplar answer on an `Answer note:` line under its Level's task, and no task
    sentence or parenthetical stating what the item asks for (Quality Standards §F item 23)?
17. **Day 1's three §D12 beats present and in order:** Phase 2's first listen carries no captions, transcript,
...
- A real-world hook tied to the source's actual topic (rotate per 0.4 and the approved plan), not abstract
  framing above the band's register. It opens by naming that topic and the kind of clip students are about to
  hear, then asks only what students can answer from that orientation, from something printed on the page, from
  their own life, or as an opinion (Quality Standards §D13). Withholding the clip's content and asking students
  to guess it is not a hook; it is the failure §D13 names. A prediction is welcome once the topic is on the
  table, and a question connecting the topic to the student's own experience is often the strongest opening,
  since every student can answer it immediately.
