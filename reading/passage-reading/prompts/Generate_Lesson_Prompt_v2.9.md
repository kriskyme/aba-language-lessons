...
18. **(New in v2.6, corrected in v2.7)** Check Section 0.10: does Phase 3 supply 2-3 distinct discussion
...
- **Phase 3 (Day 2) Oral Output:** the board-dependent moment depends on the chosen protocol (Section 0.10).
  For Fishbowl or Concentric Circles, keep a running board record of claims or evidence AS THEY ARE ACTUALLY
  SPOKEN - a single visible circle is easy to transcribe from live, and this gives the board a real-time
  transcription role that the printed stems cannot replace. For Town Hall, Jigsaw, or a small-group discussion
  carousel, a single scribe cannot transcribe several simultaneous conversations, so instead have each group
  report one notable claim or piece of evidence back to the board once its window ends, the same
  synthesize-distributed-input pattern already used in Phase 2. Either way this feeds the closing whole-class
  synthesis.
...
line to say. Two reliable options: (1) a rehearsed pair-share immediately before their turn (their small group's
discussion round, or their first rotation into the Fishbowl circle), where the student says their stem once,
quietly, to a partner and gets a thumbs-up, so their first public contribution is already a repetition; or (2) a
listening/tracking role for the stretch before their turn (a simple tally sheet marking which side each speaker
argues, or which target vocabulary word they hear - the same task an outer-circle student already has under
Section 0.10 when the protocol is Fishbowl), with one fully-prepared sentence to contribute at a natural pause
rather than an on-demand turn.
...
### 0.10 Phase 3 discussion format: scaffolded roles for every seat, in the circle or out (corrected in v2.7)

This section was added in v2.6 after classroom feedback that Fishbowl and Concentric Circles, as previously
written, do not work at realistic class sizes: both assume a class large enough that splitting off a 5-6 student
inner circle still leaves a meaningful outer circle of observers who rotate in over time, and at a realistic class
size (roughly 8-12 students) an inner circle of 5-6 already contains most of the room. v2.6 responded by
defaulting Phase 3 to simultaneous small groups instead. **Corrected in v2.7:** that diagnosis was half right - a
single static question circulating for the full 30-minute window past an unscaffolded outer circle (students
just watching) is genuinely redundant - but the fix is to scaffold both halves of the format properly, not retire
it. Fishbowl and Concentric Circles are reinstated as fully valid choices at this program's actual class size, not
reserved for a 16+-student class, once every seat has an explicit, active task.

**Inner circle (Fishbowl):** the students currently in the discussion, using their task-Level sentence stems
(Section 0.8, Part B). Size it to roughly half the class (4-6 of 8-12 students) so the outer circle stays a real,
substantial group rather than one or two leftover students.

**Outer circle (Fishbowl) - never passive listening.** Every student not currently inside the circle has a
specific, concrete task tied to what's actually being said, so "watching" produces a deliverable rather than idle
time. Choose one, matched to the lesson's Module skill, and state it explicitly in the lesson:

- a running list: write down 3 words, phrases, comparisons, or methods you hear that strike you as
  interesting or well-put (fits Describing, Narrating, Instructing)
- a tally: mark which side of the debate each inner-circle speaker takes, or how many times a target
  vocabulary word or a specific claim type comes up (fits Arguing, Evaluating)
- a one-line reaction: note one claim you agree or disagree with and a one-sentence reason, ready to say
  when you rotate in

This is distinct from, but can double as, the low-task-Level listening/tracking role in Section 0.8, Part B: every
outer-circle student gets an active task under this section; the lowest task Level's version of it is also their
differentiated-participation accommodation, not a second requirement layered on top.

**Rotate through 2-3 prompts, not one prompt for the full window.** Write 2-3 distinct, related discussion
prompts into the lesson and advance to the next one at each inner-circle changeover (roughly every 8-10
minutes across a 25-30 minute Phase 3), rather than circulating one static question the whole time. A student
rotating into the circle brings their outer-circle notes (their word list, tally, or reaction) as live material, so
later prompts build on what the room actually produced instead of restarting cold.

**Concentric Circles:** both circles talk at once (paired, rotating partners), so neither side is passive by
design and no separate outer-circle task is needed - but give it the same 2-3-prompt treatment as Fishbowl, so
each rotation of partners is working a fresh angle rather than repeating the same exchange.

**Town Hall Role-Play, Jigsaw Expert Panels, and a plain small-group discussion carousel** remain in rotation
as well and need no outer-circle scaffolding of their own, since every student is already active inside a small
group at all times; they still need the same 2-3 distinct prompts (divided across groups or stations) so
simultaneous conversations aren't all identical.

**Board-dependent moment, by protocol:** see Section 0.7's Phase 3 entry. Fishbowl and Concentric Circles keep
a running board record of claims as they're actually spoken (a single visible circle is easy to transcribe from
live). Town Hall, Jigsaw, and the small-group carousel instead build the board record from groups reporting
back once their window ends, since several simultaneous conversations can't be transcribed live by one scribe.
...
- Choose a Phase 3 protocol from Town Hall Role-Play, Fishbowl, Concentric Circles, Jigsaw Expert Panels, or a
  plain small-group discussion carousel (Section 0.10), rotating across cycles per the variety rule below.
  Scaffold discussions using explicit sentence frames (e.g., "I agree with... because...", "On the other hand..."),
  2-3 distinct discussion prompts rotated through rather than one static prompt for the full window, and, for
  Fishbowl, an explicit active task for every outer-circle student (Section 0.10) - never passive listening.
...
- Differentiated participation (see Section 0.8, Part B): the lowest task Level's role in the protocol must differ in
  participation mode, not only in stem difficulty - a rehearsed pair-share before their turn (their small group's
  discussion, or their first rotation into the circle), or a listening/tracking role with one prepared line, rather
...
- **Phase 3 Output Rotation (updated in v2.7):** Vary the oral discussion protocol for every cycle (Town Hall
  Role-Play, Fishbowl, Concentric Circles/Speed-Dating, Jigsaw Expert Panels, or a plain small-group discussion
  carousel), each carrying its own Section 0.10 scaffolding: an explicit outer-circle task and 2-3 rotating prompts
  for Fishbowl, 2-3 rotating prompts for Concentric Circles, and 2-3 prompts split across groups/stations for
  Town Hall/Jigsaw/carousel. Fishbowl and Concentric Circles are valid defaults at this program's actual class
  size (8-12 students) once scaffolded this way - they are not reserved for large classes.
