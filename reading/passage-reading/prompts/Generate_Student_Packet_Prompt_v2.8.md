# Passage Reading Student Print Formatting Prompt (v2.8)
...
**Current version: v2.8.** For the dated version history and the reasoning behind each change, see `Changelog.md`.
...
**Before You Read opens with the lesson's orientation, always printed.** The first thing under that title is one
or two plain sentences saying what the lesson is about: the topic, the situation, and what kind of text is
coming. It is not a teacher-facing term and is never stripped. Every question in that section is then answerable
from the orientation, from something printed on the page (a captioned photo, a printed choice), from the
student's own life, or as an opinion. Never print a question whose answer is in the text students have not read,
and never a line like "you do not know yet what this is" (Quality Standards §D13, Style Guide §I item 24). An
answer line under an unanswerable question is the same fault twice, since it also asks the student to write the
guess down.
...
comprehension items would split into two near-identical blocks (a Fact Finder set and the band's native-Level
set on the same content), merge them into one Task with sequential numbered questions; a vocabulary-in-context exercise stays
...
2. Before You Read opening with the printed orientation (topic, situation, kind of text) before its first
   question, and every question there answerable from that orientation, from something printed, from the
   student's own life, or as an opinion (2.3; Quality Standards §D13)?
3. Unit 1A / Unit 1B labels folded into the headings, both masthead sections carrying the label `Reading` (2.1)?
4. Objective as a can-do statement before any warm-up, and Wrap It Up connecting back to it (2.2, 2.5)?
5. Fact Finder and the native-Level set merged into one Task where they covered the same content (2.3)?
6. Worked-model box present between the Units if the objective is first tested in Unit 1B, stating its examples
   directly (2.4)?
7. Discuss It as simultaneous small groups with 2-3 unquoted, left-aligned prompts, any outer-circle task folded
   in, stems only (2.6)?
8. Idioms as the Phrase Spotlight, unnumbered and unlabeled (2.7)?
9. Annotation key present, floated beside the first paragraphs, `!` mark only when an evaluative-language
   objective exists (2.9)?
10. Refresher text as a plain bordered block, not a callout (2.10)?
11. Every hook or task photo embedded from the lesson's named asset, none placeholdered (2.11)?
12. Every `Answer note:` stripped, stops printed as questions only, no stem parenthetical stating the answer, and
