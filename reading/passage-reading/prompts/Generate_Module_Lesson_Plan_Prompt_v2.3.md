# Passage Reading Module Lesson-Plan Generation Prompt (v2.3)
...
**Current version: v2.3.** History: `Changelog.md`.
...
3. Produce a plan table, one row per lesson, with these columns:
   ...
   - **Phase 1 hook:** Visual Inquiry / Take a Side / Mystery Quote / K-W-L Chart, each used once across the
     4 lessons, adjacency rule for Lesson 1. Name, in the same cell, the **orientation** the hook opens with:
     the topic, the situation, and the kind of text it tells students about before asking anything (Lesson
     prompt 0.12, Quality Standards §D13). A hook planned without one produces a lesson that opens on a
     question about the text students have not read, which is the failure §D13 names.
...
4. After the table, report a short self-check: every genre and strategy used without repeats; no topic or theme
   ...
   every lesson 2-3 rotated prompts; every hook carrying its orientation, with no hook question answerable only
   from the unread text; no Level 1-2 task shape in the same slot in consecutive lessons; Lesson 1
