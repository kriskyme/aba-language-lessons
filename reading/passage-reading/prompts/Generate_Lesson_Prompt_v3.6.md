# Passage Reading Lesson Generation Prompt (v3.6)
...
**Current version: v3.6.** For the dated version history and the reasoning behind each change, see `Changelog.md`.
...
13. **Phase 1 hook (0.12; Quality Standards §D13):** does it name the topic, the situation, and the kind of text
    before its first question, and is every question answerable from that orientation, from something printed,
    from the student's own life, or as an opinion, with none asking who is speaking, what the text is, or what
    happened?
14. **Native-Level item set** named for this Module's own verb, never a Cause & Effect set outside Explaining
    (Quality Standards §A4)?
15. **Answer notes:** every expected answer (stops, Fact Finder, the native-Level set, matrix cells) on an `Answer
...
- **Day 1 Phase 1 activation:** a K-W-L Chart builds its K and W columns live on the board from student
  call-outs before reading (the L column after reading, on Day 1 or in Day 2 Phase 1); a Mystery Quote, having
  already named the topic and the kind of text (0.12), collects on the board what the class expects that text to
  say, do, or leave out, and checks those expectations against it after reading. A Visual Inquiry collects what
  the class can see in the picture in two columns, what it shows and what that implies, and marks which
  implications the text bears out; a Take a Side collects the tally of the printed options plus one reason per
  option as students give them. Never a board list of guesses at information the page has withheld
  (Quality Standards §D13).
...
### 0.12 The Phase 1 hook orients before it asks

Quality Standards §D13 applies in full: the hook names the topic, the situation, and the kind of text before its
first question, and no hook question's answer lies only in the text students have not read yet. Reading's four
hooks in their compliant form:

| Hook | What it does |
|---|---|
| Visual Inquiry | A captioned photograph. The caption says what the picture is; the questions ask what students can see in it and what that implies about the topic. A picture may be shown before its place is named only where the questions are answerable by looking. |
| Take a Side | The topic is named, then a printed choice with real stakes that the text later complicates. Already compliant by construction. |
| Mystery Quote | The topic and the kind of text are named first, then one line from the text is printed, and students say what that line makes them expect the text to say, do, or leave out. **Never** who is speaking, what the text is, or what happened: those are the withheld answers, and asking for them is the failure §D13 names. |
| K-W-L Chart | The topic is stated in the chart's own heading, then students record what they know and want to know. Already compliant by construction. |

After the hook's own questions, one question connecting the topic to the student's own experience is always
welcome and is often the strongest part of the lead-in, since every student can answer it from the first minute.
...
*Phase 1: Pre-Reading & Activation (15 min)*
- Open by naming the topic, the situation, and the kind of text students are about to read, then spark curiosity
  with visual prompts, open opinion questions, or a marked choice students then defend to a neighbor (Take a
  Side). Every question the hook asks is answerable from that orientation, from something printed on the page,
  from the student's own life, or as an opinion; never from the text they have not read yet (0.12, Quality
  Standards §D13). Nothing that moves the class out of its seats (Quality Standards §D11).
...
*Phase 3: Day 1 Literal Comprehension & Vocab Context Check (25 min)*
- A rapid factual check pitched at the lesson's task Levels: a Fact Finder set for the lowest task Level, and for
  the band's native Level(s) a set whose shape follows **the Module's own verb** (Quality Standards §A4), not a
  fixed default: comparison and framing for Describing, sequence and significance for Narrating, cause and effect
  for Explaining, steps and order for Instructing, criteria and verdict for Evaluating, claims and support for
  Arguing, and the equivalent for Transacting and Socializing. Name the set for what it does in this Module. A
  Cause & Effect set in a Describing or Narrating lesson is the drift §A4 forbids, not a naming choice. Every item follows Quality Standards §C; a Level 1-2 Fact Finder
...
- **Rotation across cycles** (Quality Standards §D6 governs adjacency; these are the Reading banks):
  - Reading strategy: never the same strategy in two consecutive cycles (rotate across the Reading Execution
    Rules bank).
  - Phase 1 activation hook: Visual Inquiry, Take a Side, Mystery Quote, K-W-L Chart, each in the compliant
    form 0.12 sets out.
