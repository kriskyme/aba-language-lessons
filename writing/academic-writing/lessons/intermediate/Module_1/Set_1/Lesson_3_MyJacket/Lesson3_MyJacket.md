...
reason word? If not, point to steps 2-3 before the student writes anything else. It is fine to finish only part
of the draft today; Day 2 completes it.
...
0        30                        55                      75  (minutes)
|--Phase 1--|---------Phase 2--------|--------Phase 3---------|
| Complete  | Self-edit checklist    | Share-out / final       |
| the draft | pass                   | line check              |
```
...
- **Level 2, once the one-slot frame is complete:** complete "It is ___." a second time with a word from a
  ...
  a partner. Two separate one-slot sentences, not a two-slot frame.

The teacher circulates with the same checks as Day 1: reason word present (Level 4), no stated feeling word
(Level 5), a sounded-out word rather than a copied one (Levels 2-3).
...
Transfer Check at this position. Every student leaves Day 2 with a complete draft that has been self-checked once.
...
8. Requires the centerpiece: every task is written on the Scenario object in front of the student.
9. Padded banks: the Draft Word Bank has 30 words for one- and two-word tasks; no matching lists.
10. Time balance: Levels 2-3 have the real frame, a defended choice, a second frame round on another part, and
    (Day 2) a second line, filling the same 50- and 30-minute blocks Levels 4-5 spend drafting; no add-on is
    gated on finishing early at any Level.
11. Counts: Level 4 and Level 5 drafts are counted by the students themselves and written on the page (Day 2
    Phase 1), then checked against the target table (checklist item 8); no model text is introduced in this
    lesson, so no new authored count applies.
...
16. Adjacency: the strategy (drafting from a step path / ordered detail list), the opening (silent re-look with
    two board questions), and the oral protocol (one-line share with a listener task) each differ from Lesson
...
21. Concrete prompts: the packet's "Get Ready to Write" is circle, put on desk, write three words; the two
    former reflection questions are gone; the circulation check is teacher-facing only; the self-check is
    yes/no items with marks, one list per Level printed as one Task per star (Style Guide §F).

**Writing-specific (Lesson prompt 0.3):**
1. Ceilings: the drafting targets table restates each Level's own 0.2 range; Day 2 Phase 1 has students write
   their own counts, and checklist item 8 checks them.
