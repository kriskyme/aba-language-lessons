...
modalities actually differ. See `Generate_Lesson_Prompt_v6.17.md`, Section 0.1, for the specific reason: a Reading
...
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `Generate_Lesson_Prompt_v6.17.md` | Generates one **2-day lesson**: for Beginner, one of a Set's 4 fixed positions; for Intermediate/Advanced/Proficient, one of a Module Pair's 8 positions ("THE MODULE PAIR" section), each run producing one lesson at the named position against the approved plan. Run with `shared/Program_Conventions.md` and `shared/Generation_Quality_Standards.md` pasted alongside; states only what is true of a writing lesson (the three-regime model and Scenario/Mentor Ladder in 0.1, per-Level output ceilings and required features in 0.2, the Grammar and Essay Focus banks and row-selection rules in 0.4/0.4c, self-revision evidence in 0.4a, content-volume minimums in 0.4b, checklists and Peer Editing timing in 0.6, the Module-to-form mapping in 0.9, the Module Pair arc and the per-lesson flow) and points to the shared files for everything modality-neutral. Section 0.3's self-check is the shared Quality Standards §F list plus 16 Writing-only items. Section 0 now uses `Lesson N, Day M` numbering throughout. | Current (v6.17, 2026-09-10: the draft-object introduction no longer asks students to take their object out - see `Changelog.md`); all four Intermediate Set 1 lessons regenerated against v6 2026-09-08 and Lessons 2-4 revised under v6.2's practice/draft-object split and v6.3's self-contained/concrete-prompt rules the same day (1.1.1.6, 1.1.2.8, 1.1.3.12, 1.1.4.13; see "First run" below); the Advanced Set's four lessons were generated under v4/v4.1 and retrofitted under v5 |
...
**Step 2 - Generate the lessons, one at a time.** Run `Generate_Lesson_Prompt_v6.17.md` (pasted with
...
Pair addendum and `Generate_Lesson_Prompt_v6.17.md`'s "THE MODULE PAIR"), two consecutive Modules pair into one
...
revises for effect, publishes by a pass-and-read from seats, and closes with the two Transfer Checks. **Module Pair 1-2,
...
- **Academic Writing has no §D12 beat (Quality Standards v1.17, 2026-09-10).** The new §D12 requires that any
  ...
  currently teacher-only and never printed at all (`Generate_Student_Packet_Prompt_v3.17.md` §2.6), so building
  ...
  checks" 0.6 shape (`Generate_Lesson_Prompt_v6.17.md`, the Lesson 2 Day 2 row of the fixed-output bank), which
...
- **Resolved 2026-09-10 (Quality Standards §D11, room-neutral participation):** Intermediate
  `Module_2/Set_1/Lesson_4_MyClothing/`'s gallery walk is now a pass-and-read - each final copy is passed to a
  neighbor and on again, so every student reads two pieces that are not their partner's without leaving their
  seat, keeping the per-Level card task intact (`2.1.4.0` -> `2.1.4.1`, `.md` and packet). No other Writing
  lesson carried the pattern.
...
- **Intermediate Module 2 Set 1 is untaught.** Its four lessons and packets (`2.1.x.0`; Lesson 4 at `2.1.4.1` after the 2026-09-10 §D11 pass) are first-pass
...
- **Grammar/Essay Focus Bank "Alternate" rows are not yet authored.** `Generate_Lesson_Prompt_v6.17.md`'s Section
