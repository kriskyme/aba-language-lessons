...
modalities actually differ. See `Generate_Lesson_Prompt_v6.18.md`, Section 0.1, for the specific reason: a Reading
...
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `Generate_Lesson_Prompt_v6.18.md` | Generates one **2-day lesson**: for Beginner, one of a Set's 4 fixed positions; for Intermediate/Advanced/Proficient, one of a Module Pair's 8 positions ("THE MODULE PAIR" section), each run producing one lesson at the named position against the approved plan. Run with `shared/Program_Conventions.md` and `shared/Generation_Quality_Standards.md` pasted alongside; states only what is true of a writing lesson (the three-regime model and Scenario/Mentor Ladder in 0.1, per-Level output ceilings and required features in 0.2, the Grammar and Essay Focus banks and row-selection rules in 0.4/0.4c, self-revision evidence in 0.4a, content-volume minimums in 0.4b, checklists and Peer Editing timing in 0.6, the Module-to-form mapping in 0.9, the Module Pair arc and the per-lesson flow) and points to the shared files for everything modality-neutral. Section 0.3's self-check is the shared Quality Standards §F list plus 16 Writing-only items. Section 0 now uses `Lesson N, Day M` numbering throughout. | Current (v6.18, 2026-09-14: the Beginner-only single-Set branch generalized to any Module a plan declares **standalone**, so a de-paired Intermediate/Advanced Module runs the 4-lesson publish arc; "THE MODULE PAIR" section renamed "THE ARC" - see `Changelog.md`; v6.17, 2026-09-10: the draft-object introduction no longer asks students to take their object out - see `Changelog.md`); all four Intermediate Set 1 lessons regenerated against v6 2026-09-08 and Lessons 2-4 revised under v6.2's practice/draft-object split and v6.3's self-contained/concrete-prompt rules the same day (1.1.1.6, 1.1.2.8, 1.1.3.12, 1.1.4.13; see "First run" below); the Advanced Set's four lessons were generated under v4/v4.1 and retrofitted under v5 |
| `Generate_Module_Lesson_Plan_Prompt_v4.6.md` | Plans a full **Set** (Beginner) or **Module Pair** (Intermediate/Advanced/Proficient) in one pass: Scenario, genre, each Module's Focus A/B pairing, Essay Focus direction, the Module N+1 CSV-coverage task, Mentor Ladder direction, and the position table. Run with `shared/Program_Conventions.md` and `shared/Generation_Quality_Standards.md` pasted alongside; general Rotation Log mechanics point to Conventions §F and the prompt keeps only Writing's cross-Band and within-pair rules. Writing objectives only. | Current (v4.6, 2026-09-14: a non-Beginner Band may be given a single Module when the plan declares it standalone, saved to `Module_<N>/Module<N>_<Band>_Lesson_Plan.md`) |
| `Generate_Homework_Prompt_v4.5.md` | Generates one homework assignment (grammar focus in production plus skill practice) from a lesson's content so far, at any of the timing checkpoints (Beginner: after Lesson 1-4; others: after Pair position 1-8), one section per task Level keyed to its composition regime. Run with the two shared files pasted alongside; Section 3 is Quality Standards §F plus 6 Writing items. | Current (v4.5, 2026-09-14: Beginner/Pair phrasing rekeyed to standalone Set/Pair, and the stale v4.3 footer stamp corrected; v4.4, 2026-09-09: any exemplar on an `Answer note:` line, never in the item - see `Changelog.md`); not yet run against a real lesson |
| `Generate_Assessment_Prompt_v4.3.md` | Generates a Set's (Beginner) or Module Pair's Grammar & Mechanics Check (Part A) and Writing Task (Part B). Run with `shared/Program_Conventions.md` and `shared/Generation_Quality_Standards.md` pasted alongside; keeps what is Writing-specific (new example sentences, Focus A/B and Essay Focus coverage, objective-only Part A formats, a new Scenario matched in shape to the original, target lengths from 0.2, regime-dependent sittings, the 3-point rubric) and points to Quality Standards §C for item quality, which is where Writing's assessments now pick up the distractor, padded-bank, and requires-the-centerpiece rules they never had. Self-checks are Quality Standards §F plus 4 Part A and 6 Part B items. | Current (v4.3, 2026-09-14: rekeyed to standalone Set/Pair); not yet run against a real Set or Pair |
| `Generate_Assessment_Student_Packet_Prompt_v4.1.md` | Takes a completed Assessment and produces the student handout: Grammar Check pages per Level under lettered Tasks, Writing Task cards with the rubric's Meets column as a `.checklist` (Style Guide §H.4), no answer keys or rubric tables. Run with `shared/Student_Packet_Style_Guide.md` pasted alongside; Section 5 is Style Guide §I plus 4 items. Its stale `.ans-line-tiny` mention is gone. | Current (v4.1, 2026-09-14: rekeyed to standalone Set/Pair); not yet run |
...
| `Rotation_Log_Intermediate.md`                                                    | Module 1's Set 1 (Intermediate 1-4, `1.1.x`) and Module 2's Set 1 (Intermediate 1-4 again, `2.1.x.0` except Lesson 4 at `2.1.4.1`, generated 2026-09-09), nested `## Module N` → `### Set 1`, cross-referenced as Parts 1 and 2 of Module Pair 1-2; one dated addendum per regenerated Module 1 lesson. Not a prompt itself.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | N/A (data, not a prompt)                                                                        |
| `Rotation_Log_Advanced.md`                                                        | Same, for the Advanced Band (Module 1's Set 1, Advanced 1-4, Part 1 of Module Pair 1-2, regenerated 2026-09-08 as Two Places to Study; plus a `## Module 2: Narrating` stub added 2026-09-14 holding Part 2's planned decisions, with no rows until its lessons are generated). A new Band's file is created lazily the first time a Set in that Band is generated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | N/A (data, not a prompt)                                                                        |
| `Rotation_Log_Proficient.md`                                                      | Same, for the Proficient Band: Module 1's Set 1 (Proficient 1-4, The Bag I Carry) and Module 2's Set 1 (the same bag, Proficient 1-4 at `2.1.x.0`), the complete Module Pair 1-2, both generated 2026-09-10. Created lazily on this Band's first Set. | N/A (data, not a prompt)                                                                        |
| `Rotation_Log_Beginner.md`                                                        | Same, for the Beginner Band (Module 1's Set 1, Beginner 1-4, `1.1.x.0`, planned and generated 2026-09-10). Created lazily on this Band's first Set. Beginner is exempt from Module Pairs, so its rows carry no Pair position and its Set 1 ends in a published piece. | N/A (data, not a prompt) |
...
| `lessons/advanced/Module_1/Module1_Advanced_Lesson_Plan.md`               | Plan 2.0 (2026-09-08), regenerated against `Generate_Module_Lesson_Plan_Prompt_v4.5.md`: Scenario Two Places to Study (practice object my room; draft object the student's home vs. one place from a printed list, for Dana, who needs quiet), Focus A/B at Level 5's row, Essay Focus A/B at Level 6's, per-lesson activity shapes, and a History block holding the Two Apartments plan and its two addenda. **Superseded 2026-09-14** by `lessons/advanced/ModulePair_1-2/ModulePair_1-2_Advanced_Lesson_Plan.md`, which restates Module 1's decisions unchanged and adds Module 2's; kept for its History block. | N/A (data, reconstructed from an existing Set; superseded) |
| `lessons/intermediate/Module_2/Module2_Intermediate_Lesson_Plan.md` | Plan 1.0 (2026-09-14), a **declared-standalone** Module plan, generated against `Generate_Module_Lesson_Plan_Prompt_v4.6.md`. Records the first rotation check this Set ever ran (it was skipped as Pair position 5), which ruled out **my headphones** as Proficient Modules 1-2's practice object; the new Scenario (practice object my charger, draft object my device from a printed list of eight); Module 2's Focus A/B retained unchanged from the superseded Pair plan; a fresh Mentor Ladder direction on the charger; the 4-row Set-position table; and the per-lesson activity shapes. Supersedes the Module 2 half of the Intermediate Pair plan below. | Current (plan 1.0) |
| `lessons/advanced/Module_2/Module2_Advanced_Lesson_Plan.md` | Plan 1.0 (2026-09-14), a **declared-standalone** Module plan, generated against `Generate_Module_Lesson_Plan_Prompt_v4.6.md` after Advanced Modules 1-2 were de-paired. Opens with the STANDALONE declaration and its reason. Scenario **My Last Meal, then A Meal That Went Wrong** (practice event the last thing the student made to eat; draft event one time it went wrong, the dish circled from a printed list of eight; named reader Mina, whose likely reaction carries Level 6's required feature; Transfer Check on the student's own water bottle or cup). Genre is Module 2's own row (personal narrative at Levels 4-5, reaction essay at 6-7), so the essay Levels draft a complete essay at Lesson 3. Grammar Focus A/B retained from the superseded paired plan; Essay Focus A as new content with Essay Focus B re-anchored. Four-row position table, Peer Editing at Lesson 4 Day 1, no separate verb task. The rotation check ran in full for the first time. Four lessons planned, none generated. | Current (plan 1.0) |
| `lessons/advanced/ModulePair_1-2/ModulePair_1-2_Advanced_Lesson_Plan.md` | **Module 2's half SUPERSEDED 2026-09-14** by the standalone plan above; banner-marked, do not generate from it. Module 1's half stays live and still governs `Module_1/Set_1/`. Plan 1.0 (2026-09-14), originally the single plan for the whole Advanced Pair, generated against `Generate_Module_Lesson_Plan_Prompt_v4.5.md`. Restates Module 1's already-generated decisions unchanged (Scenario Two Places to Study, practice object my room, draft object the student's home vs. one circled place, named reader Dana, Focus A/B at Level 5's row, Essay Focus B plus the working thesis) and answers the three stubs the Module 1 plan left: Module 2's hand-selected Focus A/B (reported speech and thought in narrative / non-finite openers for pacing; said / told), the Lesson 8 Module 2 verb task ("the last time you moved to a different seat, room, or place to get something done") with its combined-skill Transfer object (a pen or pencil case), and the Mentor Ladder's Lesson 5 second look per Level. Also the built-in-parts position table and Module 2's per-lesson activity shapes. Module 2's four lessons are planned but not generated. | Current (plan 1.0) |
| `lessons/beginner/Module_1/Module1_Beginner_Lesson_Plan.md` | Plan 1.1 (2026-09-10), the Beginner Band's first plan, generated against `Generate_Module_Lesson_Plan_Prompt_v4.5.md` before any Beginner lesson existed. Records the Scenario (practice object my notebook, draft object My Hat from a printed list of eight, Closing Transfer Check on the student's own shoe), the Level-1-row Focus anchoring and the untaught -s form it implies, the three-Level Mentor Ladder direction, the cross-Band check with its one "hat" recency flag, and the per-day frame-regime shape table that discharges §D10 across the Set. Addendum 1.1 corrects one shape collision found while laying out Lesson 1. | N/A (data, a plan) |
| `lessons/beginner/Module_2/Module2_Beginner_Lesson_Plan.md` | Plan 1.0 (2026-09-10), the Beginner Band's second Module, planned before any of its lessons existed. Records the Scenario (practice event my morning, draft event My Trip from a printed list of eight, Closing Transfer Check on last night), the Focus pair hand-selected against Module 2's own CSV rows (the regular past `-ed` with its four spelling rules and three sounds / time-order words at the front of a sentence; confusable pair the doubling rule), the no-irregulars rule for the taught material, the three-Level Mentor Ladder direction, the cross-Module and cross-Band checks with their three flags, and a per-day shape table that deliberately rotates three slots away from Module 1's order. | N/A (data, a plan) |
| `lessons/proficient/ModulePair_1-2/ModulePair_1-2_Proficient_Lesson_Plan.md` | Plan 1.0 (2026-09-10), the single plan for the whole Proficient Pair, generated against `Generate_Module_Lesson_Plan_Prompt_v4.5.md` before any Proficient lesson existed. Scenario The Bag I Carry (practice object my headphones, for Tomas; draft object my bag compared with itself when new, for Priya and, at Level 8, Marcus); Module 1's Focus A/B hand-selected off the Level 5 row because Advanced Module 1 had spent that row's bank entry; Module 2's Focus A/B hand-selected and checked for within-pair distinctness; Essay Focus B and the working thesis in Module 1, Essay Focus A in Module 2; the Lesson 8 verb task named. Module 2's four lessons are planned but not generated. | Current (plan 1.0) |
...
**Step 1 - Plan the Set or Module Pair.** Run `Generate_Module_Lesson_Plan_Prompt_v4.6.md` against a Band (naming
...
**Step 2 - Generate the lessons, one at a time.** Run `Generate_Lesson_Prompt_v6.18.md` (pasted with
...
**Optional - Student print handout.** Run `Generate_Student_Packet_Prompt_v3.19.md` against any completed lesson to
...
**Optional - Homework.** Run `Generate_Homework_Prompt_v4.5.md` against a lesson's content so far, at any of the
...
**Optional - Assessment.** Run `Generate_Assessment_Prompt_v4.3.md` against a completed Set (Beginner, all 4
lessons) or Module Pair (Intermediate/Advanced/Proficient, all 8 lessons) to produce Part A (Grammar & Mechanics
Check) and Part B (Writing Task), then `Generate_Assessment_Student_Packet_Prompt_v4.1.md` in the same session to
...
- `lessons/intermediate/Module_2/Set_1/Lesson_{1,2}_MyCharger/` and `Lesson_{3,4}_MyDevice/` - `Lesson{1-4}_My{Charger,Device}.md` plus `My{Charger,Device}_Intermediate_L{1-4}_Packet.html` (Module 2's Intermediate 1-4, `2.1.1.3` to `2.1.4.2`). Generated 2026-09-09 as Pair positions 5-8 on Module 1's clothing, then **de-paired and regenerated 2026-09-14** as a standalone Set on its own Scenario: the practice object (my charger) names Lessons 1-2, the draft object (my device) names Lessons 3-4, per the slug rule below. Version codes increment rather than reset (Conventions §G)
- `lessons/advanced/Module_2/Set_1/Lesson_1_MyLastMeal/`, `Lesson_2_MyLastMeal/` (`Lesson{1,2}_MyLastMeal.md`, `MyLastMeal_Advanced_L{1,2}_Packet.html`) and `Lesson_3_MealThatWentWrong/`, `Lesson_4_MealThatWentWrong/` (`Lesson{3,4}_MealThatWentWrong.md`, `MealThatWentWrong_Advanced_L{3,4}_Packet.html`) (Advanced 1-4 again, `2.1.1.0` to `2.1.4.0`; a standalone Set generated 2026-09-14 as My Last Meal, then A Meal That Went Wrong)
...
- `lessons/beginner/Module_1/Set_1/Lesson_1_MyNotebook/`, `Lesson_2_MyNotebook/` (`Lesson{1,2}_MyNotebook.md`, `MyNotebook_Beginner_L{1,2}_Packet.html`) and `Lesson_3_MyHat/`, `Lesson_4_MyHat/` (`Lesson{3,4}_MyHat.md`, `MyHat_Beginner_L{3,4}_Packet.html`) (Beginner 1-4, `1.1.1.0` to `1.1.4.0`, planned and generated 2026-09-10; the practice/draft slug convention applied from the start)
...
(`Generate_Module_Lesson_Plan_Prompt_v4.6.md`) but has not yet been run before-the-fact against a new Set/Pair, so
...
**Beginner's Set is the one Set with no Pair above it (2026-09-10).** Beginner is exempt from Module Pairing
(Conventions §C), so `lessons/beginner/Module_1/Set_1/` is a complete arc on its own: the notebook carries Focus
A and the Mentor Ladder through Lessons 1-2, the hat carries the draft from Lesson 2 Day 2 onward, and Lesson 4
is a true finale (Peer Editing at Day 1, then final copy, publishing, and the Closing Transfer Check at Day 2),
with no hand-off note and nothing kept back. It is also the first Set in this lesson type whose task Levels sit
entirely in the Guided Frame Composition regime, which makes its plan's per-day activity-shape table
(`Module1_Beginner_Lesson_Plan.md`) the working example of how Quality Standards §D10 and Lesson prompt 0.4d are
discharged across four lessons rather than inside one.

**Beginner Module 2 Set 1, added the same day.** `lessons/beginner/Module_2/Set_1/` holds the Band's second Set (My Morning, then My Trip, `2.1.1.0` to `2.1.4.0`), and it is the first place in this lesson type where the Module verb actually changes under a Band that has already run a Set: Describing's frames ("It is ___.") give way to Narrating's past-tense and sequential ones ("First, ___.", "I ___-ed.", "First, I ___-ed. Then, I ___-ed."), with a new Scenario, a new Focus pair, and a new Word Bank built from regular past verbs. Because Beginner is Pair-exempt, Module 2 carries nothing from Module 1 and finishes its own published piece at Lesson 4.

## Module Pairs

For Intermediate, Advanced, and Proficient (Beginner is exempt - see `shared/Program_Conventions.md` §C's Module
Pair addendum and `Generate_Lesson_Prompt_v6.18.md`'s "THE MODULE PAIR"), two consecutive Modules pair into one
...
This doc (produced by `Generate_Module_Lesson_Plan_Prompt_v4.6.md`) plans both Modules together in one pass:
...
**Modules 1-2, Intermediate: DE-PAIRED (2026-09-14).** The Pair was completed 2026-09-09 (Module 1's Set 1
retrofitted into Part 1, Module 2's Set 1 generated as Part 2 on Module 1's clothing across all four lessons)
and then dissolved by user direction, whose reported symptom was that all four of Module 2's lessons carried the
same topic - a correct consequence of the Pair rule, which made clothing the draft object across six consecutive
lessons. Module 2's live plan is `lessons/intermediate/Module_2/Module2_Intermediate_Lesson_Plan.md` (plan 1.0),
a standalone Set on its own Scenario (My Charger, then My Device), and its four lessons were **regenerated**
2026-09-14 at `2.1.1.1` to `2.1.4.2`: Lesson 1 opens on a hook and carries the Set's only Mentor Ladder, Lesson 2
Day 2 is real prewriting introducing the draft object, Lesson 3 drafts a complete piece, and Lesson 4 peer-edits
on Day 1 and publishes on Day 2 with one Closing Transfer Check. The Pair plan's Module 2 half carries a
SUPERSEDED banner, while its Module 1 half stays live. Module 1 Set 1 is unchanged and its Lesson 4 hand-off now
has no receiver, logged in Pending work below. **Modules 1-2,
Advanced: DE-PAIRED (2026-09-14).** Advanced Module 2 was planned that morning as Part 2 of a Pair, inheriting
Module 1's Two Places to Study Scenario and comparison-essay genre across all four lessons, and was de-paired by
the same user direction that de-paired Intermediate, before any of its lessons had been generated. Module 2's
live plan is `lessons/advanced/Module_2/Module2_Advanced_Lesson_Plan.md` (plan 1.0), a standalone Set on its own
Scenario (My Last Meal, then A Meal That Went Wrong) in its own genre; the Pair plan's Module 2 half carries a
SUPERSEDED banner, while its Module 1 half stays live. Module 1 Set 1 is unchanged and its Lesson 4 hand-off now
has no receiver, logged in Pending work below. Advanced Module 2's four lessons (Advanced 1-4 at `2.1.1.0` to
`2.1.4.0`) were **generated 2026-09-14**, each with a packet, making this the first standalone Set in this
lesson type at a Band that reaches Essay Composition: its Lesson 3 drafts a complete reaction essay in one
sitting, where the Pair model had split an essay across two Modules. See `shared/Program_Conventions.md` §C's Standing exception
(v1.16) for the status of the Module Pair convention itself, which is deferred rather than decided.

**Module Pair 1-2, Proficient: complete (2026-09-10).** This is the Band's first Academic Writing content of
any kind: before this pass there was no `lessons/proficient/`, no plan, and no `Rotation_Log_Proficient.md`. The
Pair was planned in full first, so Module 2's Focus A/B could be checked against Module 1's, then all eight
lessons were generated with a packet each. Module 1's Set (positions 1-4, `1.1.1.0` to `1.1.4.0`) teaches
noun-phrase precision and reduced clauses with Essay Focus B and the working thesis, and leaves each student
holding an unmarked part. Module 2's Set (positions 5-8, `2.1.1.0` to `2.1.4.0`) re-engages that part, teaches
narrative anteriority and then fronting and clefts alongside Essay Focus A, drafts the introduction and
conclusion around the kept bodies (or Level 5's second, narrative paragraph), and at position 8 assembles,
peer-edits, revises for effect, publishes by pass-and-read, and runs both Closing Transfer Checks. This is the
second completed Pair in the family, after Intermediate's, and the first at a band reaching Essay Composition.

**One thing this Pair makes visible that Intermediate's did not.** Because Proficient spans the 5/6 regime
boundary, Module 2's verb lands differently by Level: Level 5's Module 2 part genuinely narrates, while Levels
6-8's part is the introduction and conclusion of Module 1's comparison essay, where the only narrating is the
hook's form. That asymmetry is designed in (Conventions §C) and is the reason 0.8 requires a separate Module
N+1 verb task at Lesson 8. Each Module 2 lesson states it in its framing section rather than leaving it to be
...
- **Lesson prompt 0.4's Grammar Focus Bank rows for Levels 1-3 are written for Describing (found 2026-09-10,
  generating Beginner Module 2 Set 1).** The bank gives Level 3 the simple-present `-s` form as Focus A and
  `"and" inside a two-slot frame` as Focus B. Both are descriptive: a Narrating Set's frames are past-tense and
  sequential, so neither cell fits, and Beginner Module 2 had to hand-select its pair against Module 2's own CSV
  rows (the regular past `-ed` and its four spelling rules / time-order words at the front of a sentence). This is
  the frame-regime counterpart of the Bank's already-logged missing "Alternate" column: the Levels 1-3 rows need
  either a Module-aware column or an explicit note that they are Module 1 examples, not the only option. Until
  then, every frame-regime Set outside Module 1 hand-selects and records why.

- **Beginner Module 2 Set 1 is untaught.** Its four lessons and packets (`2.1.1.0` to `2.1.4.0`) are first-pass
  generations. Two specifics worth watching in a real class, both new to this Set rather than inherited from
  Module 1: whether Level 1's `First, ___.` slot really reads as naming an event rather than as a fragment once
  students fill it, and whether the four `-ed` spelling rules plus the three `-ed` sounds are too much grammar for
  one Beginner mini-lesson, or whether the sounds should move to Lesson 2 Day 1 alongside the doubling drill.

- **Academic Writing has no §D12 beat (Quality Standards v1.17, 2026-09-10).** The new §D12 requires that any
  support carrying the answer (the input's exact wording, a model, an exemplar) arrive only after students
  commit a response, and that the lesson end with one unsupported re-encounter. Listening/Speaking's mechanics
  were built the same day; Writing's were not. Here the support is the **Leveled Mentor Ladder**, which is
  currently teacher-only and never printed at all (`Generate_Student_Packet_Prompt_v3.19.md` §2.6), so building
  the beat means deciding whether a bounded Mentor excerpt reaches the packet *after* drafting: draft from the
  Scenario, then compare against the Mentor form for that Level, then produce one fresh instance with the model
  gone. The closest thing already in the family is the "Spell from sound ... writes each without looking, then
  checks" 0.6 shape (`Generate_Lesson_Prompt_v6.18.md`, the Lesson 2 Day 2 row of the fixed-output bank), which
  is the right shape at the wrong scale and is the model to extend. Not started; logged so §D12 does not
  quietly become a Listening/Speaking-only rule.

- **Lesson prompt 0.4's Focus row-selection rule does not resolve for Beginner, and the -s form goes untaught as
  a result (found 2026-09-10, generating Beginner Module 1 Set 1).** 0.4's "Row selection when a band spans rows"
  anchors Grammar Focus A/B at the band's own **native Paragraph Composition row** and names only Intermediate,
  Advanced, and Proficient. Beginner (task Levels 1, 2, 3) has no Paragraph Composition Level at all, so the rule
  selects nothing. The Set anchored at **Level 1's row** (the Band's calibration Level, Conventions §A), with
  Level 2's row folded into the same mini-lesson and Focus B taken from Level 3's cell, the only Focus B the
  frame regime offers; the reasoning is recorded in `Module1_Beginner_Lesson_Plan.md`. One knock-on: with Level
  3's Focus A cell unused, the simple-present **-s form is never taught in this Set**, but the Lesson prompt's
  Lesson 3 Day 2 Phase 2 still tells frame-regime Levels to "add one -s sentence about what the object does."
  `lessons/beginner/Module_1/Set_1/Lesson_3_MyHat/Lesson3_MyHat.md` runs the swap-in-a-more-exact-word half of
  that phase only and says so. The prompt is unfixed: 0.4 needs a Beginner row-selection clause, and Lesson 3 Day
  2's -s clause needs to be conditional on that form having been taught.

- **Beginner Module 1 Set 1 is untaught.** Its four lessons and packets (`1.1.1.0` to `1.1.4.0`) are first-pass
  generations, and this is the first Set in the family whose task Levels are entirely frame-regime, so its
  activity-shape rotation (Lesson prompt 0.4d) has never been checked against a real class. Review all four
  packets against a real print and classroom pass before treating them as final, the same way Intermediate
  Module 1's Set was revised through fourteen versions. Two specifics worth watching: whether Lesson 1 Day 1's
  three-round guided frame is the right amount for Level 1, and whether Lesson 3 Day 1's 45-minute drafting
  block is too long for a single frame sentence.

- **Proficient Module 2 Set 1: closed 2026-09-10.** Generated the same day it was logged as pending, at Pair
  positions 5-8 (`2.1.1.0` to `2.1.4.0`) with a packet each, against the Focus A/B, Essay Focus A direction, and
  Lesson 8 verb task the Pair plan had already fixed. Module Pair 1-2 is complete for this Band; see "Module
  Pairs" above.

- **Lesson prompt 0.2's Level 8 ceiling assumes every Module's Level 8 row is audience-based, and Module 1's is
  not (found 2026-09-10).** 0.2's Level 8 entry, and 0.3's self-check item 4, require the Scenario to name two
  distinct readers or stakes and require Level 8's output to calibrate for "two distinct named audiences." That
  matches the CSV's Level 8 rows for Modules 3 through 8, but **Module 1: Describing** at Level 8 asks instead
  for two deliberate shifts of tone, register, and abstraction plus an initial framing complicated later in the
  same text, with no audience requirement at all. Module 2: Narrating at Level 8 is likewise not audience-based
  (it asks for a subtly unreliable narrator). Proficient Module 1 Set 1 resolved this by satisfying both at once
  (two named readers, whose two paragraph groups also carry the framing and its complication), and the
  resolution is recorded in the plan and in `Rotation_Log_Proficient.md`. The prompt itself is unfixed: 0.2's
  Level 8 entry should state the audience requirement as the shape most Modules take and defer to the Module's
  own CSV row where it differs. Worth doing before any Level 8 band is generated for Modules 1 or 2 again.

- **Resolved 2026-09-10 (Quality Standards §D11, room-neutral participation):** Intermediate Module 2 Set 1's
  Lesson 4 gallery walk became a pass-and-read - each final copy is passed to a neighbor and on again, so every
  student reads two pieces that are not their partner's without leaving their seat, keeping the per-Level card
  task intact (`2.1.4.0` -> `2.1.4.1`, `.md` and packet). No other Writing lesson carried the pattern. The
  lesson that carried this fix was `Lesson_4_MyClothing/`, replaced 2026-09-14 by `Lesson_4_MyDevice/` in the
  de-pairing regeneration; the pass-and-read was carried forward into it deliberately and is named in its
  Day 2 Phase 2.
...
- **Advanced Module 2 Set 1 is untaught.** Its four lessons and packets (`2.1.1.0` to `2.1.4.0`) are
  first-pass generations, and the Band's first standalone Set; review them against a real print and classroom
  pass before treating them as final. Two things to watch specifically: **Lesson 3's 50-minute drafting block**,
  which is the first time in this lesson type that Levels 6-7 write a complete essay in one sitting rather than
  a part of one, and the **reaction essay** itself, which Lesson prompt 0.9 flags as its loosest Module-to-form
  mapping and which this Set is the first to test.
- **Two Module 1 Sets now end in a hand-off with no receiver.** A consequence of the 2026-09-14 de-pairings
  (`shared/Program_Conventions.md` §C's Standing exception), deliberately not fixed in either pass.
  **Advanced Module 1 Set 1's Lesson 4** (`1.1.4.3`) ends in a final copy of the body paragraphs under a
  working thesis, kept unmarked for a Module 2 that no longer receives it, so no lesson ever writes that
  essay's introduction or conclusion. **Intermediate Module 1 Set 1's Lesson 4** (`1.1.4.15`) is in the same
  position with its kept descriptive paragraph. Each needs either a finale of its own or an explicit decision
  that the Set ends there; whichever is chosen should be the same for both.
- **Intermediate Module 2 Set 1 is untaught, and its current content has never been reviewed at all.** The Set
  was regenerated wholesale 2026-09-14 on a new Scenario when Modules 1-2 were de-paired, so `2.1.1.3` to
  `2.1.4.2` (`Lesson_{1,2}_MyCharger/`, `Lesson_{3,4}_MyDevice/`) are first-pass generations whose only prior
  review belonged to the superseded clothing version. Lesson 1's Day 1 grammar section is the one part that has
  had a review pass (2026-09-14, `2.1.1.3`): its load was cut for the class now taking it and its Activity 3
  folded from three steps to one. The rest stands unreviewed. Review them against a real print and classroom pass before
  treating them as final, the same way Module 1's Set was revised through fifteen versions. Three things are new
  to this Set and worth watching specifically: whether the draft list of eight devices (earbuds, speaker, watch,
  calculator, flashlight, remote control, game controller, mouse) really leaves no student without one, since it
  is the first draft object in this lesson type that some students may not own; whether moving Peer Editing to
  Lesson 4 Day 1 leaves enough time to act on a partner's suggestion before the final copy on Day 2; and whether
  two separate banks in one Set (the charger Word Bank through Lesson 2 Day 1, the device Draft Word Bank from
  Lesson 2 Day 2) confuses students at the frame Levels.
- **Grammar Focus Bank, Level 4 Alternate row: a candidate exists.** The pair hand-selected for Intermediate
  ...
  A second candidate, at the **Level 5** row, comes from Advanced Module 2 (Focus A: reported speech and
  thought in narrative, with reporting verbs that carry the narrator's attitude; Focus B: non-finite
  participial and time-phrase openers with their comma; confusable pair said / told), hand-selected
  2026-09-14. Three Pairs have now hand-selected a Module N+1 pair (Intermediate, Proficient, Advanced), so
  the Alternate column has real candidates at Levels 4 and 5 waiting to be written into 0.4.
- **Grammar/Essay Focus Bank "Alternate" rows are not yet authored.** `Generate_Lesson_Prompt_v6.18.md`'s Section
